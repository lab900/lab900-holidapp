...
export const validateFirebaseIdToken: (db: admin.firestore.Firestore) => RequestHandler = (db) => async (req: RequestWithUser, res: Response, next: NextFunction) => {
  ...
  if ((!req.headers.authorization ||
    ...
    functions.logger.error(
        ...
        "Make sure you authorize your request by providing the " +
  ...
  try {
    ...
    req.user = decodedIdToken as unknown as User;
    next();
    return;
  } catch (error) {
      ...
      async (req: RequestWithUser, res: Response, next: NextFunction) => {
        try {
          ...
          const userSnapshot = await db.collection("users")
              .doc("tamim.asefi@lab900.com").get();
          ...
          functions.logger.error("Error while fetching user from " +
                    "the database:", error);
          res.status(403).send("Unauthorized");
        }
      };
